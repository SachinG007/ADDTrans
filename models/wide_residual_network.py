...
def batch_norm():
    return BatchNormalization(axis=_get_channels_axis(), momentum=0.1, epsilon=1e-4,
                              beta_regularizer=0, gamma_regularizer=1)
...
def create_wide_residual_network(input_shape, num_classes, depth, widen_factor=1, dropout_rate=0.0,
    ...
    conv1 = conv2d(n_channels[0], 5, padding = "same")(inp)  # one conv at the beginning (spatial size: 32x32)
    conv1 = MaxPooling2D(pool_size=(2, 2))(Activation('relu')(batch_norm()(conv1)))

    conv2 = conv2d(n_channels[1], 5, padding = "same")(conv1)  # one conv at the beginning (spatial size: 32x32)
    conv2 = MaxPooling2D(pool_size=(2, 2))(Activation('relu')(batch_norm()(conv2)))

    conv3 = conv2d(n_channels[1], 5, padding = "same")(conv2)  # one conv at the beginning (spatial size: 32x32)
